...
class PINNTrainer:
    """Manages the training loop, optimization, and logging."""
    def __init__(self, model: BurtonPINN, config: Config, scaler: DataScaler):
        self.model = model.to(config.device)
        self.config = config
        self.scaler = scaler

        # Standard MSE Loss (More robust and readable)
        self.criterion = nn.MSELoss()

        # Optimizer for NN weights and Physics Params (Minimization)
        ...
        # Optimizer for Adaptive Weights (Maximization/Gradient Ascent)
