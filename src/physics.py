...
class BurtonPINN(nn.Module):
    ...
    def __init__(self, config: Config, u_0: float, t_0: float,
        ...
        # Adaptive Loss Weight (Lambda) or balancing physics vs data loss
    ...
    def compute_physics_loss(self, t_p: torch.Tensor, q_p: torch.Tensor) -> torch.Tensor:
        """Calculates ODE residual: R = dy/dt + y/tau - a*q"""
        t_p.requires_grad_(True)
        y_pred = self.forward(t_p)

        # Automatic Differentiation for time derivative
        dydt = torch.autograd.grad(
            outputs=y_pred, inputs=t_p,
            grad_outputs=torch.ones_like(y_pred), create_graph=True
        )[0]

        residual = dydt + (y_pred / (self.tau + 1e-9)) - (self.a_param * q_p)
